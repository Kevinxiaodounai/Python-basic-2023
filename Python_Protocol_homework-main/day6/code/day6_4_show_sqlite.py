...
import numpy as np
from new_homework.day6.code.tools.mat_line import mat_line
...
router_if_infos = session.query(InternfaceMonitor.device_ip,
                                InternfaceMonitor.interface_name).group_by(InternfaceMonitor.device_ip,
                                                                           InternfaceMonitor.interface_name).all()
...
for device_ip, interface_name in router_if_infos:
    ...
    device_if_info = session.query(InternfaceMonitor).\
        ...
        filter(InternfaceMonitor.record_datetime >= one_hours_before)
